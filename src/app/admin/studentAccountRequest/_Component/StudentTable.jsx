...
import { ChevronLeft, ChevronRight, CircleCheckBig, Search, Trash } from 'lucide-react';
...
export default function StudentTable() {
  ...
  const { data, isLoading } = useGetAllStudentRequestQuery({
    limit: 10,
    page: currentPage,
    searchText,
  });
  ...
  const [updateStatus, { isLoading: updating }] = useUpdateStudentStatusMutation();
  ...
  const handledeniedUser = async (values) => {
    const payload = {
      userId: values._id,
      status: 'denied',
    };
    try {
      const res = await updateStatus(payload).unwrap();
      if (res.success) {
        toast.success('  User request denied successfully!');
      }
    } catch (error) {
      toast.error(error?.data?.message);
    }
        ...
        <div className="flex-center-start gap-x-3">
          ...
          <Tooltip title="Block User">
            <CustomConfirm
              ...
              loading={updating}
              onConfirm={() => handleBlockUser(record, 'approved')}
            >
              <button>
                <CircleCheckBig color="#32CD32" size={22} />
              </button>
            </CustomConfirm>
          </Tooltip>
          <Tooltip title="Delete User">
            <CustomConfirm
              title={'Denied User Request'}
              description={`Are you sure to denied this user request ?`}
              loading={Deleteting}
              onConfirm={() => handledeniedUser(record)}
            >
              <button>
